...
<style>
    ...
    .alerta-amarelo {
        background: linear-gradient(135deg, #3d3000, #1a1500);
...
def detectar_alertas(df):
    ...
    if len(df) >= 3:
        if df["Pressao"].tail(3).mean() >= 4:
            alertas.append(("🌡️", "Pressão interna persistentemente alta", "alto"))
            gravidade += 2
        if df["Humor"].tail(3).mean() <= 2:
            alertas.append(("😔", "Humor consistentemente baixo", "alto"))
            gravidade += 2
        if df["Sono"].tail(3).mean() <= 2:
            alertas.append(("😴", "Qualidade de sono muito baixa", "medio"))
            gravidade += 1
        if df["Irritabilidade"].tail(3).mean() >= 4:
            alertas.append(("😤", "Irritabilidade elevada nos últimos dias", "medio"))
            gravidade += 1
        if df["Bateria"].tail(3).mean() <= 2:
            alertas.append(("🔋", "Bateria social esgotada", "medio"))
            gravidade += 1
        if df["Nevoa"].tail(3).mean() <= 2:
            alertas.append(("🧩", "Névoa mental persistente — dificuldade de foco", "medio"))
            gravidade += 1
...
st.sidebar.success(f"👤 {nome_usuario}")
...
if not df_todos.empty and "codigo_usuario" in df_todos.columns:
    ...
    if not df_user_rem.empty:
        remedios_salvos = df_user_rem["remedios"].dropna().iloc[-1] if not df_user_rem["remedios"].dropna().empty else ""

remedios_input = st.text_area("Remédios que você toma:",
...
with st.form("diario_form"):
    ...
    with c2:
        ...
        nevoa = st.select_slider("🧩 Foco Mental", options=[1,2,3,4,5], value=3)
        pressao = st.select_slider("🌡️ Pressão Interna", options=[1,2,3,4,5], value=1)
    atualizar_rem = st.checkbox("Atualizar meus remédios junto com este registro")
...
if st.button("📈 Gerar Relatório dos Últimos 7 Dias"):
    ...
    if df_user.empty:
        st.markdown('<div class="alerta-vermelho"><h3 style="color:#ff4444">⚠️ Sem registros encontrados</h3><p style="color:#fff">Você ainda não fez nenhum registro.</p></div>', unsafe_allow_html=True)
        st.stop()
    ...
    if df_7dias.empty:
        st.markdown('<div class="alerta-vermelho"><h3 style="color:#ff4444">⚠️ Sem participação esta semana</h3><p style="color:#fff">Nenhum registro nos últimos 7 dias.</p></div>', unsafe_allow_html=True)
        st.stop()
    ...
    emojis = {"Humor":"😊","Sono":"😴","Pressao":"🌡️","Irritabilidade":"😤","Bateria":"🔋","Nevoa":"🧩"}
    for i, m in enumerate(metricas):
        if m in df_7dias.columns:
            media = df_7dias[m].mean()
            delta = df_7dias[m].iloc[-1] - df_7dias[m].iloc[0] if len(df_7dias) > 1 else 0
            cols[i].metric(
                label=f"{emojis[m]} {m}",
                value=f"{media:.1f}/5",
    ...
    df_plot = df_7dias[["data"] + metricas].copy()
    df_long = df_plot.melt(id_vars="data", value_vars=metricas, var_name="Métrica", value_name="Nível")

    fig = go.Figure()
    for metrica in metricas:
        df_m = df_long[df_long["Métrica"] == metrica]
        fig.add_trace(go.Scatter(
            x=df_m["data"], y=df_m["Nível"],
            mode="lines+markers",
            name=metrica,
            line=dict(color=CORES_METRICAS.get(metrica, "#ffffff"), width=2.5),
            marker=dict(size=8),
        ))
    ...
    fig.update_layout(
        ...
        legend=dict(bgcolor="#0e1117", bordercolor="#2d2d3f"),
        xaxis=dict(gridcolor="#2d2d3f", title="Data"),
        yaxis=dict(gridcolor="#2d2d3f", range=[0,6], tickmode="linear", dtick=1, title="Nível (1-5)"),
        hovermode="x unified",
        margin=dict(l=20, r=20, t=40, b=20)
    )
    st.plotly_chart(fig, use_container_width=True)

    # ── Gráfico radar (perfil da semana) ─────────────────────
    ...
    fig_radar.update_layout(
        polar=dict(
            bgcolor="#1a1d27",
            radialaxis=dict(visible=True, range=[0,5], gridcolor="#2d2d3f", color="#ffffff"),
            angularaxis=dict(gridcolor="#2d2d3f", color="#ffffff")
        ),
    ...
    if alertas:
        itens_html = "".join([f'<li style="margin:8px 0"><b>{e}</b> {txt}</li>' for e, txt, _ in alertas])
        nivel = "ALTO RISCO" if gravidade >= 4 else "ATENÇÃO"
        cor_borda = "#ff4444" if gravidade >= 4 else "#ffcc00"
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,#2d1500,#1a0e00);
                    border:2px solid {cor_borda};border-radius:12px;padding:20px;margin-top:15px">
            <h3 style="color:{cor_borda};margin-top:0">⚠️ {nivel} — Sinais Detectados</h3>
            <p style="color:#ccc"><b>Para o profissional de saúde:</b> O paciente <b>{nome_usuario}</b> apresentou:</p>
            <ul style="color:#ddd">{itens_html}</ul>
            <p style="color:#aaa;font-size:0.9em">Recomenda-se revisão dos procedimentos terapêuticos.</p>
